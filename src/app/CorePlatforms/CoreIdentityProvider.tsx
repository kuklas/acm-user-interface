...
// Note: No clusters column as this is for a single cluster (Core platforms perspective)
...
const CoreIdentityProvider: React.FunctionComponent = () => {
  ...
  const handleSelectProviderType = (providerType: string) => {
    ...
    if (providerType === 'LDAP') {
      navigate('/core/user-management/identity-providers/add/ldap');
    }
        ...
        <Table aria-label="Identity providers table" variant="compact">
          <Thead>
            <Tr>
              ...
              />
              <Th width={30}>Identity provider</Th>
              <Th width={20}>Type</Th>
              <Th width={20}>Status</Th>
              <Th width={30}>Connected Users</Th>
            </Tr>
          </Thead>
          <Tbody>
            {paginatedProviders.map((provider) => (
              <Tr key={provider.id}>
                ...
                />
                <Td dataLabel="Identity provider" width={30} style={{ textAlign: 'left' }}>
                  <Button
                    variant="link"
                    isInline
                    onClick={() => navigate(`/core/user-management/identity-providers/${provider.name}`)}
                    style={{ paddingLeft: 0 }}
                  >
                    {provider.name}
                  </Button>
                ...
                <Td dataLabel="Connected Users" width={30}>{provider.users}</Td>
              </Tr>
            ))}
          </Tbody>
        </Table>
